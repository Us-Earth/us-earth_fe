  ...
  const addImageFile = (e) => {
    ...
    if (e.target.files.length + previewImg.length < 6) {
      if (e.target.files) {
        ...
        filesInArr.map( async (file, index) => {
          if (acceptImageFiles.includes(file.type)) {
            if (file.size < 21000000) {
              ...
              try {
                ...
                const convertedBlobFile = new File([compressedFile], file.name, { type: file.type, lastModified: Date.now() });
